...
def test_readme_block_extraction() -> None:
    s = (
        """Of course. Here is an updated `README.md` that better reflects the project's features and structure.\n"""
        """\n"""
...
def test_multiple_new_files_stay_separate() -> None:
    ...
    _assert(
        abs(files[0]["start"] - files[0]["end"]) > len("export default function A()..."),
...
def test_general_run_tests() -> None:
    ...
    _assert(
        files_nested[0]["language"] in ("md", "markdown", "plain"), "language not detected for md"
    )
    _assert(
        "echo hello" in files_nested[0]["code"] and "export const x" in files_nested[0]["code"],
